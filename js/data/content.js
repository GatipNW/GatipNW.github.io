  ...
  th: {
    ...
    resume: {
      ...
      summary: [
        ...
        'ไม่ได้มีแค่ทักษะแปล — ถ้าบริษัทอยากเข้าตลาดไทย ผมทำได้ครบทั้งสายพานในคนเดียว: แปลและ transcreate → ตั้งชื่อเกม/เลือกฟอนต์ไทย → วางแผน PR และการตลาด → ดูแลคอมมูนิตี้และโซเชียล → ติดต่อสื่อและอินฟลูฯ (เครือข่ายรวมกว่า 1.5 ล้านผู้ติดตาม) → ยืนบูทและเป็นล่ามหน้างาน — โดยไม่ต้องจ้างเอเจนซีเพิ่ม',
        'คู่ภาษา ญี่ปุ่น→ไทย เป็นคู่ที่หาคนยาก — สตูดิโอญี่ปุ่นส่วนใหญ่ยังไม่มีคนไทยประจำในทีม การมีคนที่อ่านต้นฉบับญี่ปุ่นออกและเขียนไทยให้คนไทยอ่านสนุกได้ในคนเดียว จึงตัดขั้นตอนแปลผ่านภาษาอังกฤษทิ้งไปได้ทั้งขั้น',
      ...
      facts: [
        ...
        { k: 'วีซ่าทำงาน', v: 'เข้าเกณฑ์ 技術・人文知識・国際業務 — ป.ตรี Business Japanese ตรงกับสายงาน และเคยผ่านขั้นตอนเข้าทำงานที่ญี่ปุ่นมาแล้ว' },
      ...
      colophon: 'เว็บนี้ผมออกแบบและคุมทิศทางเอง เขียนโค้ดโดยมี Claude เป็นคู่เขียนหลัก · Vanilla JS + Canvas 2D ไม่ใช้ framework · ฉากและภาพประกอบทุกใบเจนด้วยสคริปต์ Python · ผมเทสและตัดสินใจทุกอย่างเอง — ตัวเว็บก็นับเป็นผลงานอีกชิ้นหนึ่ง',
  ...
  en: {
    ...
    resume: {
      ...
      summary: [
        ...
        'Not just translation — if a studio wants to enter the Thai market, I can run the whole pipeline alone: translation and transcreation → Thai title naming and font selection → PR and marketing planning → community and social → media and influencer outreach (a network reaching 1.5M+ followers) → booth staffing and on-site interpreting — with no agency needed on top',
        'Japanese→Thai is a genuinely scarce pair: most Japanese studios still have no Thai speaker in-house. One person who reads the Japanese source and writes Thai that Thai players enjoy removes the entire English-pivot step',
      ...
      facts: [
        ...
        { k: 'Work visa', v: 'Eligible for the Engineer/Specialist in Humanities/International Services status — my B.A. in Business Japanese matches the role, and I have been through the process of working in Japan before' },
      ...
      colophon: 'I designed and art-directed this site myself and wrote the code with Claude as my main pair-programmer · vanilla JS + Canvas 2D, no framework · every scene and illustration is generated by Python scripts · the testing and every design call were mine — the site itself is part of the portfolio.',
  ...
  ja: {
    ...
    resume: {
      ...
      summary: [
        ...
        '翻訳だけではありません。タイ市場への参入をお考えでしたら、一連の流れを一人で回せます: 翻訳・トランスクリエーション → タイ語タイトルの命名とフォント選定 → PR・マーケ設計 → コミュニティとSNS運用 → メディア／インフルエンサーへの働きかけ（総フォロワー150万人超のネットワーク）→ ブース対応と現場通訳 — 追加でエージェンシーを挟む必要がありません',
        '日本語→タイ語は人材が非常に少ない言語ペアです。多くの日本のスタジオには社内にタイ語話者がおらず、日本語の原文を直接読み、タイのプレイヤーが楽しめるタイ語を書ける人材がいれば、英語を経由する工程そのものを省けます',
      ...
      facts: [
        ...
        { k: '移住', v: '🇯🇵 日本への移住可能' },
        { k: '就労ビザ', v: '「技術・人文知識・国際業務」の要件に該当（ビジネス日本語学科卒で職務内容と一致・日本での就業手続き経験あり）' },
        { k: '現住所', v: 'タイ・ノンタブリー県' },
      ],
      ctaHead: '🎉 全ゾーンご覧いただきありがとうございました',
      ctaBody: '募集中のポジションと合いそうでしたら、ぜひ一度お話しさせてください。',
      updated: '最終更新: 2026年7月',
      colophon: '本サイトは企画・デザイン・アートディレクションを自分で行い、コードは Claude を主なペアプログラマーとして書きました · フレームワークは使わず Vanilla JS + Canvas 2D · 背景やイラストはすべて Python スクリプトで生成 · テストと設計判断はすべて自分で行っています — サイト自体もポートフォリオの一部です。',
