...
[English CV](https://gatipnw.github.io/assets/resume-nipith-wongsirikul.pdf)
...
私が担当したのは、コンセプトとアートディレクション（月の中のスタジオ、藍・和紙・朱・金と
いう和の夜の配色、「部屋が地味だ」という問題を小物ではなく**光**で解決すると決めたこと）、
すべての設計判断と修正指示、そして 3 言語ぶんの文章とその事実確認です。e スポーツの項目に
「プロチームには全敗した」と明記させたのも私です。実際そうだったからです。

そしてテスト。自分でプレイして問題を報告しました——操作が滑らかでない、クリック後に歩行が
止まる、画面がぼやける、キャラクターが家具をすり抜ける、和紙パネル上の文字が濃色で読めない、
モバイルでレイアウトが崩れる。直るまで差し戻しています。

Claude が担当したのは、それらの判断を JavaScript と Python に落とし込み、
報告したバグの原因を突き止めることでした。
