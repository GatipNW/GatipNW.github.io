...
先に正直に書いておきます。**コードは Claude（Anthropic）を主なペアプログラマーとして
書きました。** 私はローカライズの専門職であり、ソフトウェアエンジニアではありません。
「他人の言葉を預かる仕事」を売り込むポートフォリオで、全部ひとりで書いたふりをするのは
筋が通らないと考えました。

私が担当した部分:

- **コンセプトとアートディレクション** — 月の中にあるスタジオ、藍・和紙・朱・金という
  和の夜の配色、アイソメトリックではなく HD-2D という選択、そして「部屋が地味だ」という
  問題を小物の追加ではなく**光**で解決すると決めたこと。
- **すべての設計判断と修正指示。** ロゴ背後の光芒を削除、プレイヤーの影を削除、
  ポリンは小さいの 9 体ではなく大きいの 5 体、語学のパワーバーは廃止して実際の
  使用場面に変更、ベクターのアーケード筐体は維持——といった判断です。
- **3 言語ぶんの文章すべてと、その事実確認。** e スポーツの項目で「プロチームには
  全敗した」と明記させたのも私です。実際そうだったからです。
- **テスト。** 自分でプレイして問題を報告しました。操作が滑らかでない、クリック後に
  歩行が止まる、画面がぼやける、ポリンが家具をすり抜ける、和紙パネル上の文字が
  濃色で読めない、モバイルでレイアウトが崩れる——直るまで差し戻しています。

Claude が担当したのは、それらの判断を JavaScript と Python に落とし込むことと、
私が報告したバグの原因を突き止めることです。

このプロジェクトの判断はすべて私のものです。分担したのはタイピングの部分です。
